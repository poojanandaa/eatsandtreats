		...
		<c:when test="${not empty cartLines}">
			<table id="cart" class="table table-hover table-condensed">
						...
						<td data-th="Item">
							<div class="row">
								<div class="col-sm-2 hidden-xs">
									<img src="${images}/${cartLine.item.code}.jpg" alt="${cartLine.item.name}" class="img-responsive dataTableImg"/></div>
								<div class="col-sm-10">
									<h4 class="nomargin">${cartLine.item.name} 
										<c:if test="${cartLine.available == false}">
											<strong style="color:red">(Not Available)</strong> 
										</c:if>
									</h4>
									
									<p>Description : ${cartLine.item.description}
								</div>
							</div>
						</td>
						<td data-th="Price"> &#8377; ${cartLine.buyingPrice} /-</td>
						<td data-th="Quantity">
							<input type="number" id="count_${cartLine.id}" class="form-control text-center" value="${cartLine.itemCount}" min="1" max="3">
						</td>
				...
				<tfoot>
					...
					<tr>
						<td><a href="${contextRoot}/show/all/items" class="btn btn-warning"><span class="glyphicon glyphicon-chevron-left"></span> Continue Shopping</a></td>
